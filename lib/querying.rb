...
def select_value_and_count_of_most_prolific_species
  ...
  COUNT(characters.species) AS number_of_species
  FROM characters
  GROUP BY species
...
def select_series_title_with_most_human_characters
  "SELECT title FROM (
    SELECT series.title AS title, COUNT(characters.series_id) AS number_of_species
      FROM series
      INNER JOIN characters
        ON series.id = characters.series_id
      WHERE characters.species = 'human'
      GROUP BY characters.series_id
      ORDER BY number_of_species DESC LIMIT 1);"
end
